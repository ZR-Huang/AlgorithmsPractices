...
represent different tasks. Tasks could be done without 
original order. Each task could be done in one interval. 
For each interval, CPU could finish one task or just be idle.

However, there is a non-negative cooling interval n that 
means between two same tasks, there must be at least 
...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ...
        while tasks_counter:
            count = n + 1
            keys = sorted(tasks_counter.keys(), key=lambda key: tasks_counter[key], reverse=True)
            for key in keys:
                if tasks_counter[key] == 0:
                    tasks_counter.pop(key)
                    continue
                tasks_counter[key] -= 1
                count -= 1
                ans += 1
